...
int inference_yolov8_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results) //三个传参表示模型、图像和结果文件
{
    int ret;
    ...
    const float nms_threshold = NMS_THRESH;      // 默认的NMS阈值，交并比值（两个框重叠面积/两个框总面积）
    const float box_conf_threshold = BOX_THRESH; // 默认的置信度阈值
    int bg_color = 114; //进行填充时的灰色背景值
    ...
    memset(od_results, 0x00, sizeof(*od_results));  //清空存放结果文件的容器
    ...
    if (dst_img.virt_addr != NULL) {
        ...
        cv::Mat show_img(dst_img.height, dst_img.width, CV_8UC3);
    ...
    ret = rknn_inputs_set(app_ctx->rknn_ctx, app_ctx->io_num.n_input, inputs);  //这里将输入数据从CPU内存传输到NPU内存，进行推理准备
    if (ret < 0)
    {
